...
2. **DTM 是最难维度。** 诊疗管理能力（鉴别诊断、方案合理性、依据解释）显著弱于 MI 与 HC；医学专用模型在 DTM 上平均 Macro-F1 仅约 **22.80%**，且 Hamming 与 Macro-F1 差距最大，条目级一致率可能**高估**实际可靠性。
...
5. **科室差异显著。** 耳鼻喉科在 DTM（诊断依据解释）上薄弱；眼科在 HC（共情与问诊组织）上易将简洁误判为冷漠；精神科与肿瘤科因高不确定性与情绪负担，模型易将表面完整误判为充分。

6. **数据来源可验证。** 对本仓库 `qa.jsonl` 的患者首问在春雨医生平台检索，约 **7.7%**（145/1880 条可检索样本）可精确匹配到原始咨询记录，说明数据来自真实在线问诊场景，但也意味着大量对话经过改写或无法公开检索到原文。
...
- **数据来自特定语言与医疗体系**（中文在线问诊），跨语言、跨机构泛化需进一步验证。
